...
def load_dataset(data_path, args, logger):
    if args.target is None: args.target = args.years
    ptr = np.load(os.path.join(data_path, args.years, 'his.npz'))
    trans = np.load(os.path.join(data_path, args.target, 'his.npz'))
    expert_idx = pd.read_pickle(os.path.join(data_path, 'expert_dict.pk'))
    ...
    for cat in ['train', 'val', 'test']:
        if cat == 'train':
            ...
            idx = np.load(os.path.join(data_path, args.target, 'idx_' + cat + '.npy'))
            dataloader[cat + '_loader'] = DataLoader(trans['data'][..., :args.input_dim], idx, \
                                         args.seq_len, args.horizon, args.bs, logger)
    dataloader['expert_idx'] = expert_idx 
